    ...
    **Note**: Be sure to shuffle the support examples and labels
    ...
    if prompt_mode == "babi":
        for i in permutation:
            prompt += "{} In the {}. ".format(support_inputs[i], support_labels[i])
        prompt += "{} In the". format(test_input)

    elif prompt_mode == "none":
        for i in permutation:
            prompt += "{} {} ".format(support_inputs[i], support_labels[i])
        prompt += test_input

    elif prompt_mode == "tldr":
        for i in permutation:
            prompt += "{} TL;DR: {} ".format(support_inputs[i], support_labels[i])
        prompt += "{} TL;DR:".format(test_input)

    elif prompt_mode == "custom":
        for i in permutation:
            prompt += "{} therefore {}. ".format(support_inputs[i], support_labels[i])
        prompt += "{} therefore".format(test_input)
    ### END CODE HERE ###
...
def do_sample(
    ...
    with torch.inference_mode():
        token = None
        input = input_ids
        while len(sampled_tokens) < max_tokens and token not in stop_tokens:
            token = model(input).logits[:, -1, :].argmax()
            if token not in stop_tokens:
                sampled_tokens.append(token)
                input = torch.cat((input[0], token.unsqueeze(0)), dim=0).unsqueeze(0)
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            icl_prompt = get_icl_prompts(support_x, support_y, test_input, prompt_mode)
                            tokenized_prompt = torch.tensor(tokenizer.encode(icl_prompt)).to(DEVICE)
                            sample = do_sample(model, tokenized_prompt.unsqueeze(0), stop_tokens, max_tokens)
                            decoded_prediction = tokenizer.decode(sample)
                            # END YOUR CODE
