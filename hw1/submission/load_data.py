...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        N_folders = random.sample(self.folders, self.num_classes)
        ...
        labels_and_images = get_images(N_folders, np.eye(self.num_classes), num_samples=self.num_samples_per_class)
        ...
        query_set = [(label, image) for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class == 0]
        support_set = [(label, image) for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class != 0]
        np.random.shuffle(query_set)
        labels_and_images = support_set + query_set

        labels = np.array([label for label, _ in labels_and_images])
        images = np.array([self.image_file_to_array(image, self.dim_input) for _, image in labels_and_images])
        ...
        images = np.reshape(images, (self.num_samples_per_class, self.num_classes, 784)).astype(np.float32)
        labels = np.reshape(labels, (self.num_samples_per_class, self.num_classes, self.num_classes)).astype(np.float32)
