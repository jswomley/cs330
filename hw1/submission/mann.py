...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        labels = torch.cat([input_labels[:, :-1, :, :], zero_labels], dim=1)

        x = torch.cat((input_images, labels), dim=-1)
        x = torch.reshape(x, (-1, self.samples_per_class*self.num_classes, self.num_classes+784))
        ...
        x = torch.reshape(x, (-1, self.samples_per_class, self.num_classes, self.num_classes))
    ...
    def loss_function(self, preds, labels):
        ...
        query_labels = labels[:, -1, :, :].reshape(-1, self.num_classes).argmax(dim=1)
