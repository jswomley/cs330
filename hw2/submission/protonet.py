...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for i, task in enumerate(task_batch):
            ...
            # accuracy_query_batch.
            support_feats = self._network(images_support)

            classes = torch.unique(labels_support, sorted=True)
            prototypes = torch.stack([support_feats[labels_support == c] for c in classes]).mean(dim=1)

            support_probs = -torch.square(torch.cdist(support_feats, prototypes, p=2))

            query_feats = self._network(images_query)
            query_probs = -torch.square(torch.cdist(query_feats, prototypes, p=2))

            loss_batch.append(F.cross_entropy(query_probs, labels_query))
            accuracy_support_batch.append(util.score(support_probs, labels_support))
            accuracy_query_batch.append(util.score(query_probs, labels_query))
