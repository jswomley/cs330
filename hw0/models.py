...
class MultiTaskNet(nn.Module):
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        for i in range(len(layer_sizes)-1):
            mlp_layers += [nn.Linear(layer_sizes[i], layer_sizes[i+1]), nn.ReLU()]
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        score = torch.cat([U, Q, torch.mul(U,Q)], axis=1)
        for layer in self.mlp_layers:
            score = layer(score)

        # Matrix Factorization Head
        B = self.B(item_ids)
        predictions = torch.sum(torch.mul(U,Q), axis=1) + B.squeeze(-1)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Regression head
        score = torch.cat([U_reg, Q_reg, torch.mul(U_reg, Q_reg)], axis=1)
        for layer in self.mlp_layers:
            score = layer(score)

        # Matrix Factorization Head
        B = self.B(item_ids)
        predictions = torch.sum(torch.mul(U_fact,Q_fact), axis=1) + B.squeeze(-1)
        score = score.squeeze(-1)
        ### END CODE HERE ###
        return predictions, score
